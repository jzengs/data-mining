...
if __name__=="__main__":
	...
	save_path=filename.split(".")[0]+"_fpgrowth.txt"

	data_set=load_data('dataset/chess.csv')
	algorithm='fp_growth'
	#data_set=load_data('dataset/groceries.csv')
	min_support_tr=int(len(data_set)*min_support)
	fp=Fp_growth()
	start=time.time()
	#L,support_data=fp.generate_L(data_set, min_support_tr)
	rule_list = fp.generate_R(data_set, min_support_tr, min_conf)
	end=time.time()
